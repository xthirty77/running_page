...
def check_today_running_records():
    ...
    try:
        ...
        # 查询今日的跑步记录
        records = session.query(Activity).filter(
        ...
        if has_records:
            ...
            for record in records_list:
                print(f"- {record['name']}: {record['distance']}km, {record['moving_time']}")
...
def send_email_notification(has_records, records_count, records_list):
    ...
    try:
        ...
        msg['Subject'] = "🏃‍♂️ 跑步提醒 - " + ("今日已完成跑步" if has_records else "今日还未跑步")
...
def main():
    ...
    # 发送通知
    serverchan_sent = send_serverchan_notification(has_records, records_count, records_list)
